...
module Itv(B:BOUND) = (struct
  ...
  let split ((l,h):t) (m:bound) : (t bot) * (t bot) =
    check_bot (l,m), check_bot (m,h)

  let split_integer ((l,h):t) (m:bound) : (t bot) * (t bot) =
    let ll,hh = B.floor m, B.ceil m in
    let hh = if B.equal ll hh then B.add_up hh B.one else hh in
    check_bot (l,ll), check_bot (hh,h)
